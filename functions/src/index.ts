 ...
 * }
 */
export const createProfessional = functions.https.onCall(
  async (data, context) => {
    ...
    if (!email || !password || !name || !phone || commissionRate === undefined || !ownerId) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Todos os campos são obrigatórios: email, password, name, phone, commissionRate, ownerId'
      );
    }
    ...
    if (typeof commissionRate !== 'number' || commissionRate < 0 || commissionRate > 100) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'commissionRate deve ser um número entre 0 e 100'
      );
    }
    ...
    if (context.auth.uid !== ownerId) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'Você só pode criar profissionais para sua conta'
      );
    }
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        'internal',
        'Erro ao criar profissional. Tente novamente mais tarde.'
      );
    }
  }
);
 ...
 * }
 */
export const deactivateProfessional = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
    ...
    if (!professionalId || !ownerId) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'professionalId e ownerId são obrigatórios'
      );
    }
    ...
    if (context.auth.uid !== ownerId) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'Você só pode desativar profissionais da sua conta'
      );
    }
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        'internal',
        'Erro ao desativar profissional. Tente novamente mais tarde.'
      );
    }
  }
);
...
const buildSlots = ({
  ...
  open: string;
  close: string;
  appointments: Array<{ id: string; data: FirebaseFirestore.DocumentData }>;
}): string[] => {
  const slots: string[] = [];
  ...
  for (let current = openMin; current + duration <= closeMin; current += duration) {
    ...
    const hasConflict = appointments.some((appointment) => {
      ...
      const startField = appointment.data.date as admin.firestore.Timestamp | undefined;
      if (!startField) return false;
      const appointmentStart = startField.toDate();

      const endField = appointment.data.endTime as admin.firestore.Timestamp | undefined;
      const appointmentDuration =
        typeof appointment.data.duration === 'number' && appointment.data.duration > 0
          ? appointment.data.duration
          : 60;
      const appointmentEnd = endField
        ? endField.toDate()
        : new Date(appointmentStart.getTime() + appointmentDuration * 60_000);
...
export const getPublicAvailability = functions.https.onCall(
  async (data: {
    userId?: string;
    date?: string;
    duration?: number;
    ...
    if (!userId || !dateStr) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'userId e date sao obrigatorios'
      );
    }

    const duration =
      typeof requestedDuration === 'number' && requestedDuration > 0 && requestedDuration <= 600
        ? requestedDuration
        : 60;
...
export const getReschedulingData = functions.https.onCall(
  async (data: { token?: string }) => {
    ...
    const expiresAt = appointmentData.reschedulingExpiresAt as admin.firestore.Timestamp | undefined;
    if (!expiresAt || expiresAt.toDate().getTime() < Date.now()) {
      throw new functions.https.HttpsError(
        'failed-precondition',
        'Link de reagendamento expirado'
      );
    }
    ...
    const appointmentDuration =
      typeof appointmentData.duration === 'number' && appointmentData.duration > 0
        ? appointmentData.duration
        : typeof serviceData?.duration === 'number' && serviceData.duration > 0
          ? serviceData.duration
          : 60;

    const dateField = appointmentData.date as admin.firestore.Timestamp | undefined;
...
export const rescheduleWithToken = functions.https.onCall(
  async (data: { token?: string; date?: string; time?: string }) => {
    const token = data?.token?.trim();
    ...
    if (!token || !date || !time) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'token, date e time sao obrigatorios'
      );
    }
    ...
    if (daySchedule?.enabled === false) {
      throw new functions.https.HttpsError(
        'failed-precondition',
        'Dia indisponivel para agendamentos.'
      );
    }
    ...
    const hasConflict = sameDayAppointments.docs.some((doc) => {
      if (doc.id === appointmentDoc.id) return false;
      const current = doc.data();
      if (current.status === 'cancelado') return false;

      const currentStart = (current.date as admin.firestore.Timestamp | undefined)?.toDate();
      if (!currentStart) return false;

      const currentEndField = current.endTime as admin.firestore.Timestamp | undefined;
      const currentDuration =
        typeof current.duration === 'number' && current.duration > 0 ? current.duration : 60;
      const currentEnd = currentEndField
        ? currentEndField.toDate()
        : new Date(currentStart.getTime() + currentDuration * 60_000);
